"""Base classes for IPSuite nodes."""
...
class Mapping(ProcessAtoms):
    ...
    def run(self):
        """Execute the main method to run the node."""
        self.atoms = []
        self.molecules = []
    ...
    def get_molecules_per_configuration(self) -> typing.List[typing.List[ase.Atoms]]:
        ...
        for atoms in self.atoms:
            molecules_per_configuration.append(
                self.molecules[start : start + len(atoms)]
            )
            start += len(atoms)
    ...
    def forward_mapping(
        ...
        """Map the atoms to a coarse-grained representation."""
    ...
    def backward_mapping(
        ...
        """Map the coarse-grained representation back to the original representation."""
        raise NotImplementedError


class CheckBase(zntrack.Node, abc.ABC):
    ...
    def check(self, atoms: ase.Atoms) -> bool:
        """Check whether a simulation should be stopped."""
        ...

    @abc.abstractmethod
    def get_value(self, atoms: ase.Atoms):
        """Return the metric that is tracked for stopping."""
        ...

    @abc.abstractmethod
    def get_quantity(self) -> str:
        """Return the quantity that is tracked for stopping."""
        ...

    def __str__(self):
        """Return status as string representation."""
        return self.status
